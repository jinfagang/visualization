...
private:
  ...
  message_filters::Subscriber<cti_msgs::DetectedObjectArray> *detection_filter_subscriber_;
  ...
  message_filters::sync_policies::ApproximateTime<sensor_msgs::Image,
    cti_msgs::DetectedObjectArray> SyncPolicyT;
  ...
  SyncedDetectionsCallback(
    ...
    const cti_msgs::DetectedObjectArray::ConstPtr &in_range_detections);

  bool IsObjectValid(const cti_msgs::DetectedObject &in_object);

  cv::Mat ObjectsToRects(cv::Mat in_image, const cti_msgs::DetectedObjectArray::ConstPtr &in_objects);
